...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.conv_out = nn.Conv1d(filters, n_classes, kernel_size=1)
        self.data_bn = nn.BatchNorm1d(in_channels * A.size(1))
        self.tcns = nn.ModuleList([copy.deepcopy(
            SingleStageModel(num_layers=10, num_f_maps=64, dim=feature_dim, num_classes=args.num_classes,
                             dil=args.temporal_dil)) for s in
                                   range(4)])
